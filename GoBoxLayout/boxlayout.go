...
func main() {
    ...
    mainwin.Layout().SetSizePolicy(ui.ExpandingWidth, ui.ExpandingHeight)
    
    layoutLabels := mainwin.Layout()
    var lblTest [12]*ui.GoLabelObj
    // Add some controls
    for i := 0; i < 12; i++ {
        lblTest[i] = ui.GoLabel(layoutLabels, "Long String TestLabel" + strconv.Itoa(i + 1))
